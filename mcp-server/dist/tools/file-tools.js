...
function applyNamingRule(template, ctx) {
    const now = new Date();
...
function suggestNamingRule(ctx) {
    const parts = [];
...
async function buildNamingContext(revit, suffix, counterSeed) {
...
// ── Tool registration ───────────────────────────────────────────────────────
export function registerFileTools(server, revit) {
    server.tool("suggest_save_as_name", "Preview what a file will be named given a naming rule template, using the currently open model's project info. Use this before save_as_with_rule to confirm the output name.", {
        namingTemplate: z
            .string()
            .optional()
            .describe("Template with tokens like {projectNumber}_{originalName}_RVT{revitVersion}_{date}. Leave blank to auto-suggest."),
        suffix: z
            .string()
            .optional()
    ...
    server.tool("save_as_with_rule", "Save the current Revit model to a new location using a naming rule. Typically called after opening and upgrading a file.", {
        targetFolder: z
            .string()
            .describe("Absolute path to the destination folder, e.g. C:\\Projects\\Upgraded"),
        namingTemplate: z
            .string()
            .optional()
            .describe("Naming template. Leave blank to auto-suggest based on project info."),
        suffix: z
            .string()
            .optional()
        ...
        worksharingMode: z
            ...
            .optional()
            .default("detach")
            .describe('"detach" = save as a non-workshared local copy, "preserve" = keep worksharing, "none" = central-file behaviour'),
    }, async ({ targetFolder, namingTemplate, suffix, counterSeed, overwrite, compact, worksharingMode, }) => {
        try {
            const ctx = await buildNamingContext(revit, suffix, counterSeed);
            const template = namingTemplate ?? suggestNamingRule(ctx);
    ...
    server.tool("open_and_upgrade", "Open a Revit file from an older version and upgrade it to the current installed Revit version. Use save_as_with_rule afterwards to save to the correct location.", {
        sourcePath: z
            .string()
        ...
        audit: z
            ...
            .optional()
            .default(true)
            .describe("Run Audit on open to repair corrupted elements (recommended)"),

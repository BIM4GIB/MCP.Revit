...
export function registerModelTools(
  ...
  server.tool(
    "revit_ping",
    "Check if the Revit bridge add-in is running and reachable",
    {},
    async () => {
  ...
  server.tool(
    "get_model_info",
    "Get high-level information about the currently open Revit model (title, file path, version, project info, element count)",
    {},
    async () => {
      try {
  ...
  server.tool(
    "get_categories",
    "List all element categories present in the current Revit model",
    {},
    async () => {
      try {
  ...
  server.tool(
    "get_levels",
    "List all levels in the current Revit model with their elevations",
    {},
    async () => {
      try {
        ...
        const text = levels
          ...
          .join("\n");
        return {
          content: [{ type: "text" as const, text: text || "No levels found." }],
        };
      } catch (err) {
        return {
          content: [{ type: "text" as const, text: `Error: ${revitError(err)}` }],
